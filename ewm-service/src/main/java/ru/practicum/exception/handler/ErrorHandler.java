...
public class ErrorHandler {
    ...
    public ResponseEntity<Object> handleNotValidMethodArgument(final MethodArgumentNotValidException e) {
        ...
        return buildResponseEntity(new ApiError(HttpStatus.BAD_REQUEST, "Body parameters are not valid.", e.getFieldError().getDefaultMessage(), LocalDateTime.now()));
    }
    ...
    public ResponseEntity<Object> handleConstraintViolationException(final ConstraintViolationException e) {
        ...
        return buildResponseEntity(new ApiError(HttpStatus.BAD_REQUEST, "Constraints of the request parameters are not met.", e.getMessage(), LocalDateTime.now()));
    }
    ...
    public ResponseEntity<Object> handleConflictException(final ConflictException e) {
        log.warn(e.getMessage());
        return buildResponseEntity(new ApiError(HttpStatus.CONFLICT, "Request is conflicting with set application constraints.", e.getMessage(), LocalDateTime.now()));
    }
    ...
    public ResponseEntity<Object> handleForbiddenException(final ForbiddenException e) {
        log.warn(e.getMessage());
        return buildResponseEntity(new ApiError(HttpStatus.FORBIDDEN, "Action is forbidden by application constraints.", e.getMessage(), LocalDateTime.now()));
    }
